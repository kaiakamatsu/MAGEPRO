...
option_list = list(
  ...
  make_option("--functional", action="store", default=NA, type='character',
              help="Path to txt file with functional information [required for MAGEPRO]")
)
...
IMPACT_split <- function(IMPACT_file, BIM, colnum){
	# PURPOSE: find the indices of potentially predictive snps (used to split sumstats)
	# IMPACT_file = path to IMPACT file 
	# BIM = BIM matrix
	# colnum = column number of IMPACT scores
	# ASSUMES "SNP" COLUMN EXISTS WITH RSID
	# RETURN: groupings: vector of strings, "nonzero" stores indices of important SNPs and "zero" stored indices of unimportant ones 
	IMPACT_scores <- as.data.frame(fread(IMPACT_file))
	IMPACT_scores_reordered <- IMPACT_scores[match(BIM[,2], IMPACT_scores$SNP),]
	IMPACT_threshold <<- quantile(IMPACT_scores_reordered[,colnum], 0.90) #CHANGE THIS TO TAKE TOP X%
	nonzero <<- which(IMPACT_scores_reordered[,colnum] >= IMPACT_threshold)
        zero <<- which(IMPACT_scores_reordered[,colnum] < IMPACT_threshold)
	groupings <- c()
        if (!identical(integer(0), nonzero)){
                groupings <- append(groupings, "nonzero")
        }
...
# --- BIN SNPs BASED ON phyloP SCORES
EVO_split <- function(EVO_file, BIM, colnum){
        # PURPOSE: find the indices of potentially predictive snps (used to split sumstats)
        # EVO_file = path to EVO constraint file
        # BIM = BIM matrix
        # colnum = column number of phyloP scores
        # ASSUMES "SNP" COLUMN EXISTS WITH RSID
        # RETURN: groupings: vector of strings, "nonzero" stores indices of important SNPs and "zero" stored indices of unimportant ones
        EVO_scores <- as.data.frame(fread(EVO_file))
        EVO_scores_reordered <- EVO_scores[match(BIM[,2], EVO_scores$SNP),]
        EVO_threshold <<- 1.3
	constrained <<- which(EVO_scores_reordered[,colnum] >= EVO_threshold)
        neutral <<- which(EVO_scores_reordered[,colnum] < EVO_threshold & EVO_scores_reordered[,colnum] > (-1*EVO_threshold))
        evolving <<- which(EVO_scores_reordered[,colnum] <= (-1*EVO_threshold))
	groupings <- c()
        if (!identical(integer(0), constrained)){
                groupings <- append(groupings, "constrained")
        }
        if (!identical(integer(0), neutral)){
                groupings <- append(groupings, "neutral")
        }
	if (!identical(integer(0), evolving)){
                groupings <- append(groupings, "evolving")
        }
        return (groupings)
}
...
if ( "MAGEPRO" %in% model ){
        if (!is.na(opt$functional)){
                if ( !file.exists(opt$functional) ){
                        cat( "ERROR: --functional file does not exist \n" , sep='', file=stderr() )
                        cleanup()
                        q()
                }
        }else{
                cat( "ERROR: Cannot perform MAGEPRO without the --functional flag\n" , sep='', file=stderr() )
                cleanup()
                q()
        }
}
...
# --- IMPACT
IMPACT_threshold <- NA
#groups <- IMPACT_split( opt$functional, genos$bim, 5) 
# ---
# --- EVO
EVO_threshold <- NA
groups <- EVO_split( opt$functional, genos$bim, 2)
# ---
...
save( wgt.matrix, snps, cv.performance, hsq, hsq.pv, N.tot , wgtmagepro, cf_total, avg_training_r2_single, avg_training_r2_meta, avg_training_r2_magepro, var_cov, avg_cor, SINGLE_top1, IMPACT_threshold, EVO_threshold, file = paste( opt$out , ".wgt.RDat" , sep='' ) )
