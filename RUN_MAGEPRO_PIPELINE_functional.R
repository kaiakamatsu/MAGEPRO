...
option_list = list(
  ...
  make_option("--functional", action="store", default=NA, type='character',
              help="Path to txt file with functional information (IMPACT or evo constraint) split by chromosome (prefix minus chr number) [required for MAGEPRO]")
)
...
arg = paste("sbatch MAGEPRO_PIPELINE/5_RunJobs_functional.sh", batch, opt$ge, opt$scratch, opt$intermed_dir, opt$out, opt$PATH_plink, opt$PATH_gcta, opt$sumstats_dir, opt$sumstats, opt$models, opt$ss, opt$cell_meta, opt$resid, opt$hsq_p, opt$lassohsq, opt$hsq_set , opt$crossval, opt$verbose, opt$noclean, opt$save_hsq, opt$functional, sep = " ") # you may have to edit this script "5_RunJobs.sh" to suit your HPC cluster
system( arg , ignore.stdout=SYS_PRINT, ignore.stderr=SYS_PRINT )
}
