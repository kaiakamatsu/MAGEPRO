...
#SBATCH --mem=2G
#SBATCH -t 04:00:00
#SBATCH -J MAGEPRO_impact_fitonce
#SBATCH -A csd832
#SBATCH -o ../working_err/MAGEPRO_impact_fitonce.%j.%N.out
#SBATCH -e ../working_err/MAGEPRO_impact_fitonce.%j.%N.err
#SBATCH --export=ALL
...
then
    ...
    TMP=$tmpdir/${gene}
    OUT=$weights/${gene}
    Rscript MAGEPRO_IMPACT_fit_alphas_betas_atonce.R --gene $gene --bfile $wd/$gene --covar $intermed/Covar_All.txt --tmp $TMP --out $OUT --PATH_gcta $gcta --PATH_plink ${plink_exec} --sumstats_dir $sumstats_dir --sumstats $sumstats --models $models --ss $ss --cell_meta $cell_meta --resid $resid --hsq_p $hsq_p --lassohsq $lassohsq --hsq_set $hsq_set --crossval $crossval --verbose $verbose --noclean $noclean --save_hsq $save_hsq --IMPACT ${IMPACT}${chr}.txt
    rm $wd/$gene.* 
fi
