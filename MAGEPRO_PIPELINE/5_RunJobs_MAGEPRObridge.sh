...
#SBATCH --mem=2G
#SBATCH -t 06:00:00
#SBATCH -J MAGEPRObridge
#SBATCH -A ddp412
#SBATCH -o ../working_err/MAGEPRObridge.%j.%N.out
#SBATCH -e ../working_err/MAGEPRObridge.%j.%N.err
#SBATCH --export=ALL
...
then
    ...
    TMP=$tmpdir/${gene}
    OUT=$weights/${gene}
    Rscript MAGEPRO_bridge.R --gene $gene --bfile $wd/$gene --covar $intermed/Covar_All.txt --tmp $TMP --out $OUT --PATH_gcta $gcta --PATH_plink ${plink_exec} --sumstats_dir $sumstats_dir --sumstats $sumstats --models $models --ss $ss --cell_meta $cell_meta --resid $resid --hsq_p $hsq_p --lassohsq $lassohsq --hsq_set $hsq_set --crossval $crossval --verbose $verbose --noclean $noclean --save_hsq $save_hsq --IMPACT ${IMPACT}${chr}.txt
    #Rscript MAGEPRO_bridge_meta_target.R --gene $gene --bfile $wd/$gene --covar $intermed/Covar_All.txt --tmp $TMP --out $OUT --PATH_gcta $gcta --PATH_plink ${plink_exec} --sumstats_dir $sumstats_dir --sumstats $sumstats --models $models --ss $ss --cell_meta $cell_meta --resid $resid --hsq_p $hsq_p --lassohsq $lassohsq --hsq_set $hsq_set --crossval $crossval --verbose $verbose --noclean $noclean --save_hsq $save_hsq --IMPACT ${IMPACT}${chr}.txt
    rm $wd/$gene.* 
fi
