...
#SBATCH -J FLEX
#SBATCH -A ddp412
#SBATCH -o workingerr/FLEX.%j.%N.out
#SBATCH -e workingerr/FLEX_ab.%j.%N.err
#SBATCH --export=ALL
...
mkdir $tmpdir
weights=/expanse/lustre/scratch/kakamatsu/temp_project/GTExTEMP/weights_flexmeta_rerun/$tissue #change directory to store weights
mkdir -p $weights
...
batchfile=$home_dir/genes_assign_Whole_Blood2.txt #change this before redoing. 
...
then
    ...
    Rscript /expanse/lustre/projects/ddp412/kakamatsu/eQTLsummary/multipopGE/ComputeMultiPopWeights.R --gene $gene --tissue $tissue --bfile $wd/$gene --covar $home_dir/Covar_All_${tissue}.txt --hsq_p 1 --tmp $TMP --out $OUT --models lasso --PATH_gcta /expanse/lustre/projects/ddp412/kakamatsu/fusion_twas-master/gcta_nr_robust --gemmaout ${gene}_${tissue} --PATH_gemma /expanse/lustre/projects/ddp412/kakamatsu/gemma-0.98.5-linux-static-AMD64 --verbose 2 --PATH_plink /expanse/lustre/projects/ddp412/kakamatsu/plink
    rm $wd/$gene.* 
