...
if (opt$batch) {
  ...
  for (batch in batches) {
    arg = paste("sbatch MAGEPRO_PIPELINE/5_batch_RunJobs.sh", batch, opt$ge, opt$scratch, opt$intermed_dir, opt$out, opt$PATH_plink, opt$PATH_gcta, opt$sumstats_dir, opt$sumstats, opt$models, opt$ss, opt$resid, opt$hsq_p, opt$lassohsq, opt$hsq_set , opt$crossval, opt$verbose, opt$noclean, opt$save_hsq, opt$ldref_pt, opt$prune_r2, opt$threshold_p, opt$ldref_PRSCSx, opt$dir_PRSCSx, opt$phi_shrinkage_PRSCSx, opt$pops, opt$impact_path, opt$ldref_dir, opt$ldrefs, opt$out_susie, opt$skip_susie, opt$n_threads, sep = " ") # you may have to edit this script "5_RunJobs.sh" to suit your HPC cluster
    system( arg , ignore.stdout=SYS_PRINT, ignore.stderr=SYS_PRINT )
  }
} else {
