...
for ( cv in 1:opt$crossval ) { 		
	...
	folds_train = cut(seq(1,n_train),breaks=5,labels=FALSE) # split sample indices into two groups (1 = betas, 2 = alphas)
	...
	indx_alphas = which(folds_train==1,arr.ind=TRUE)
...
folds_full = cut(seq(1,n_full),breaks=5,labels=FALSE) # split sample indices into two groups (1 = betas, 2 = alphas)
...
indx_alphas = which(folds_full==1,arr.ind=TRUE)
