...
randomgenes=random_genes_LD_h${heritability}
mkdir $randomgenes
...
do 
	...
	while true; do
		...
		indexeurcausal=$(python3 check_LD.py ${randomgenes}/AFR_1KG_chr${chr}_${random_position} $index_causal)
		status=$?
                if [ $status -eq 1 ]; then
	...
	for numafr in "${numsafr[@]}"; do
		...
		simgeno=simulated_genotypes_LD_${numafr}_h${heritability}

		sumstatsdir=sumstats_LD_${numafr}_h${heritability}

		tempdir=temp_LD_${numafr}_h${heritability}
		...
		python3 Sim_SumStats_LD.py ${randomgenes}/EUR_1KG_chr${chr}_${random_position} 500 EUR ${simgeno}/simulated_genotypes_EUR.csv 1 $indexeurcausal $i $heritability $numafr $sumstatsdir $tempdir
		...
		python3 Sim_Model_LD.py ${randomgenes}/AFR_1KG_chr${chr}_${random_position} $numafr AFR ${simgeno}/simulated_genotypes_AFR.csv ${sumstatsdir}/sumstats_EUR.csv 1 $index_causal $i $heritability $tempdir
	done
done
