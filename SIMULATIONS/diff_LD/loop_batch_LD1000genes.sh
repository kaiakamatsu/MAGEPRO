...
#jobs=(1 6 7)
for job in "${jobs[@]}"; do
    out=/expanse/lustre/projects/ddp412/kakamatsu/MAGEPRO_SIMULATIONS_DIR/MAGEPRO_simulations_highLD${job}
    rm -rf $out
    mkdir $out
    ...
    for h in "${heritability[@]}"; do
        sbatch batch_sim_LD1000genes.sh $afr_sizes $h $eur_geno_prefix $afr_geno_prefix $amr_geno_prefix $threads $out_results
    done
done
