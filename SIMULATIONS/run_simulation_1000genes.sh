...
randomgenes=random_genes_h${heritability}
mkdir $randomgenes
...
do 
	...
	while true; do
		...
		#hapmapsnps.txt is a file of snps we are interested in simulating LD from
	...
	for numafr in "${numsafr[@]}"; do
		...
		simgeno=simulated_genotypes_${numafr}_h${heritability}

		sumstatsdir=sumstats_${numafr}_h${heritability}

		tempdir=temp_${numafr}_h${heritability}
		...
		python3 Sim_SumStats.py ${randomgenes}/EUR_1KG_chr${chr}_${random_position} 500 EUR ${simgeno}/simulated_genotypes_EUR.csv 1 $index_causal $i $heritability $numafr $sumstatsdir $tempdir
		...
		python3 Sim_Model.py ${randomgenes}/AFR_1KG_chr${chr}_${random_position} $numafr AFR ${simgeno}/simulated_genotypes_AFR.csv ${sumstatsdir}/sumstats_EUR.csv 1 $index_causal $i $heritability $tempdir
	done
done
