...
#return a list of snps that should be kept or flipped
...
# P value threshold for colocalisation analysis - if two traits share a causal variant (colocalised), they could also share a causal mechanism? 
...
# maximum number of permutations to perform for each feature 
...
print("match sumstat and ld ref")
...
print("flip Z scores for sumstats")
...
for ( w in 1:nrow(wgtlist) ) {
	...
	#---
	no_variance_snps = which(apply(genos$bed, 2, function(x) all(x == x[1])))
	#---
	...
	m.keep = !is.na(m)
	snps = snps[m.keep,]
	if (!is.matrix(wgt.matrix)){
		wgt.matrix = as.matrix(t(wgt.matrix))
	}
	...
	# already done when computing multipop weights? 
	...
	if ( length(cur.Z) == 0 ) {
		...
		if ( !cur.FAIL ) {
			...
			#print(cur.twasr2pred)
			print("compute twas Z")	
	...
	if ( cur.FAIL || sum(eqtlmod) == 0 || length(topeqtl) == 0 || is.na(topeqtl) || is.null(rownames(wgt.matrix)) ) {  #add in a condition here - if nrow of wgts is 1
