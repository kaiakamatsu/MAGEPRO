...
filename = "eQTLs_1000sims_Nov/Nov_sims_eQTLeffectsizes_0.75rg_SNPpolygen5_tissue%s_sim%s.txt.gz" % (ti+1,sim) #text file of causal eqtl effect sizes; matrix dim = genes x snps
...
#info about true cis h2, this is equal to sum of eqtl betas^2 for a gene. 
...
bim, fam, G_all = read_plink("1KG_HM3_chr1", verbose=False) #this is just the plink files for the relevant snps, e.g. the ones around 1 Mb of the simulated gene. 
G_all = G_all.T
bim = np.array(bim)

filename = "Simulated_eQTL_Cohort_for_ImputedExpression.txt.gz" ##read in the EUR or AFR simulated peole (whoever you want to do gene model in) You only need the SNPs that are in the 1 Mb window around the simulated gene. 
...
#I think you can skip this first part because it's just formatting the true effect size info
#You will just read in a simple vector or array since it's python. 
...
#    b_qtls_pass[x_var,0] = b_qtls[x,1] #need new x-var where chunk is the gene.
    ...
    for nqtl in samplesizes:
        ...
        h2 = 0.05 #recode this if you change it. 
        coef, h2g, hsq_p, r2all, r2train, gexpr = sim_eqtl(gene, ti, sim, Z_qtl, nqtl, b_qtls_pass, float(h2)) #this runs gcta and lasso 
