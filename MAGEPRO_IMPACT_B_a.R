...
cv.all = cv.all[ cv.sample , ] #reorder
...
for ( cv in 1:opt$crossval ) { 		
	...
	#0a. split training cohort into two (train Betas and alphas)
	n_train = nrow(cv.train) # the order of the full cohort is already randomized before
	folds_train = cut(seq(1,n_train),breaks=2,labels=FALSE) # split sample indices into two groups (1 = betas, 2 = alphas)
	indx_betas = which(folds_train==1,arr.ind=TRUE)
	cv.train_betas = cv.train[indx_betas,]
        cv.train_betas[,3] = scale(cv.train_betas[,3]) 
        cv.file_betas = paste(opt$tmp,".cvbetas",sep='')
        write.table( cv.train_betas , quote=F , row.names=F , col.names=F , file=paste(cv.file_betas,".keep",sep=''))
        arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$tmp," --keep ",cv.file_betas,".keep --out ",cv.file_betas," --make-bed",sep='')
        system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)

	indx_alphas = which(folds_train==2,arr.ind=TRUE)
	cv.train_alphas = cv.train[indx_alphas,]
        cv.train_alphas[,3] = scale(cv.train_alphas[,3])

	#0b. train betas 
	pred.wgt_train = weights.lasso( cv.file_betas , lasso_h2 , snp=genos$bim[,2] )
        if ( sum(is.na(pred.wgt_train)) == length(pred.wgt_train)) {
                if ( opt$verbose >= 1 ) cat("MAGEPRO: LASSO pushed all weights to 0, using top1 as backup \n")
                pred.wgt_train = weights.marginal( genos$bed[ cv.sample[-indx][ indx_betas ],] , as.matrix(cv.train_betas[,3,drop=F]) , beta=T )
                pred.wgt_train[ - which.max( pred.wgt_train^2 ) ] = 0
        }
        if (length(pred.wgt_train) == 1){
                pred.wgt_train <- t(pred.wgt_train) # 1 snp in the cis window -> transpose for "matrix" mult
        }


	if (ext2 > 0){	
	#1a. format glmnet input
	eq <- matrix(0, nrow = nrow(genos$bed[ cv.sample[-indx][ indx_alphas ] , ]), ncol = ext2+1)
	eq[,1] <- genos$bed[ cv.sample[-indx][ indx_alphas ] , ] %*% pred.wgt_train
	for (c in 1:length(wgt2)){
		eq[,(c+1)] <- genos$bed[ cv.sample[-indx][ indx_alphas ] , ] %*%  eval(parse(text = wgt2[c]))
	}	
	...
	y <- cv.glmnet(x = eq , y = cv.train_alphas[,3], alpha = 0.5, nfold = 5, intercept = T, standardize = T)
	cf = coef(y, s = "lambda.min")[2:(ext2+2)]
	predtext <- "cf[1]*pred.wgt_train"
...
n_full = nrow(cv.all) # the order of the full cohort is already randomized before
folds_full = cut(seq(1,n_full),breaks=2,labels=FALSE) # split sample indices into two groups (1 = betas, 2 = alphas)
indx_betas = which(folds_full==1,arr.ind=TRUE)
cv.all_betas = cv.all[indx_betas,]
cv.all_betas[,3] = scale(cv.all_betas[,3])
cv.file_all_betas = paste(opt$tmp,".cvallbetas",sep='')
write.table( cv.all_betas , quote=F , row.names=F , col.names=F , file=paste(cv.file_all_betas,".keep",sep=''))
arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$tmp," --keep ",cv.file_all_betas,".keep --out ",cv.file_all_betas," --make-bed",sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)

indx_alphas = which(folds_full==2,arr.ind=TRUE)
cv.all_alphas = cv.all[indx_alphas,]
cv.all_alphas[,3] = scale(cv.all_alphas[,3])

#0b. train betas 
pred.wgt_half = weights.lasso( cv.file_all_betas , lasso_h2 , snp=genos$bim[,2] )
if ( sum(is.na(pred.wgt_half)) == length(pred.wgt_half)) {
	if ( opt$verbose >= 1 ) cat("LASSO pushed all weights to 0, using top1 as backup \n")
        pred.wgt_half = weights.marginal( genos$bed[ cv.sample[ indx_betas ],] , as.matrix(cv.all_betas[,3,drop=F]) , beta=T )
        pred.wgt_half[ - which.max( pred.wgt_half^2 ) ] = 0
}
if (length(pred.wgt_half) == 1){
	pred.wgt_half <- t(pred.wgt_half) # 1 snp in the cis window -> transpose for "matrix" mult
}

if (ext2 > 0){
w1 <- genos$bed[ cv.sample[ indx_alphas ],] %*% pred.wgt_half
eqfull <- matrix(0, nrow = length (w1), ncol = ext2 + 1)
eqfull[, 1] <- w1
num = 2
for (w in wgt2){	
	eqfull[,num] <- genos$bed[ cv.sample[ indx_alphas ],] %*% eval(parse(text = w))
	num = num+1
}	
#run ridge regression to find optimal coefficients and compute multipop weight
yfull <- cv.glmnet(x = eqfull , y = cv.all_alphas[,3], alpha = 0.5, nfold = 5, intercept = T, standardize = T)
cf_total = coef(yfull, s = "lambda.min")[2:(ext2+2)] # coef can be all pushed to 0	
predtextfull <- "cf_total[1]*pred.wgt_half"
