...
# --- PROCESS SUMSTATS AND SCALE BY IMPACT SCORES

datasets_process_impact <- function(bim, dataset, file, wgts, IMPACT_file){
	# PURPOSE: process sumstats by flipping signs according to effect allele and matching snps, scale by IMPACT scores
	# bim = bim file of snps	
	# dataset = name of sumstat dataset
	# file = file path to sumstat (make sure it is properly formatted)
	# wgts = current list of wgts 
	# IMPACT_file = path to IMPACT scores file 
	# RETURN: updated list of wgts, including the one that was processed in this function
	table <- fread(file, select = c(2, 3, 4, 5)) 
	for (k in 1:nrow(table)){
		match=which(bim$V2 == table[k, 1])
...
if ( ("MAGEPRO" %in% model) | ("META" %in% model) | ( !is.na(opt$cell_meta) )){
	if (!is.na(opt$ss)){
		...
		cat( "ERROR: Cannot perform MAGEPRO, sample-size weighted meta-analysis, or cell-type meta-analysis without the --ss flag\n" , sep='', file=stderr() )
                cleanup()
                q()
	}
}
...
for (w in wgts){
	...
	ss_sumstats <- append(ss_sumstats, h[[dataset]])
}
total_ss_sumstats <- sum(ss_sumstats)
}


if ("MAGEPRO" %in% model){

# CREATE PENALTY.FACTOR VECTOR FOR GLMNET BASED ON SAMPLE SIZES OF DATASETS
	# no penalty for target cohort weight, penalty inversely proportional to sample size of datasets (larger sample size, smaller penalty)
ss_inverse <- 1/ss_sumstats
pf <- append(0, ss_inverse) # cv.glmnet() internally scales penalty.factor to sum to the number of variables (no need to scale here)

wgt2 <- c() #magepro weights

# SCALE WEIGHTS BY IMPACT SCORES
for (d in datasets){
        name <- strsplit(d, split="[.]")[[1]][2]
        wgt2 <- datasets_process_impact(genos$bim, name, eval(parse(text = d)), wgt2, opt$IMPACT)
}
...
for ( cv in 1:opt$crossval ) { 		
	...
	#1b. when geno %*% wgt -> all 0 (snps have no variaion among people in sumstat) -> remove sumstat
	zero_cols <- colSums(eq == 0) == nrow(eq) 
	if (any(zero_cols)) {
  		eq <- eq[, !zero_cols]
  		ext2 <- ext2 - sum(zero_cols)
  		wgt2 <- wgt2[-(which(zero_cols) - 1)]
		pf <- pf[-which(zero_cols)]
	}

	#2. run ridge regression to find optimal coefficients for each dataset
	y <- cv.glmnet(x = eq, y = cv.train[,3], alpha = 0.5, nfold = 5, intercept = T, standardize = T, penalty.factor = pf)
...
yfull <- cv.glmnet(x = eqfull , y = pheno[,3], alpha = 0.5, nfold = 5, intercept = T, standardize = T, penalty.factor = pf)
