...
magepro_matrix <- matrix(nrow=ncol(genos$bed))
for (w in wgt2){
	current_matrix = diag(eval(parse(text = w)))
	magepro_matrix <- cbind(magepro_matrix, current_matrix)
}
# remove first column of NA created during matrix initialization
magepro_matrix <- magepro_matrix[,-1]
