...
datasets_process_impact <- function(bim, dataset, file, wgts, IMPACT_file){
	...
	minimal <- which(IMPACT_scores_reordered[,5] < 0.01)
	IMPACT_scores_reordered[minimal,5] <- 0
...
for ( cv in 1:opt$crossval ) { 		
	...
	y <- cv.glmnet(x = eq , y = cv.train[,3], alpha = 0.5, nfold = 5, intercept = T, standardize = T)
...
yfull <- cv.glmnet(x = eqfull , y = pheno[,3], alpha = 0.5, nfold = 5, intercept = T, standardize = T)
